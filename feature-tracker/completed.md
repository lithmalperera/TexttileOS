...
- **Type:** Domain feature (Catalog)
- **Evidence:** `V5__catalog_bom.sql` (one BOM per product via unique product_id, unique material per BOM, positive-quantity check) plus `V6__add_bom_version.sql` (the `@Version` column missed in V5; applied as a new migration because applied migrations are immutable). `Bom`/`BomItem` parent-child entities with `@OneToMany` cascade/orphan-removal and cross-aggregate references by ID; `BomService` with get-or-create, material validation (active only), smart replace (update-in-place for existing materials — prevents the unique-constraint violation caused by delete+insert of the same key), `retainMaterials` diffing, requirement calculation, and view composition with material details; quantity scale normalized to 6 decimals in the domain (`BR-QTY-005`); REST API `GET/PUT /products/{id}/bom` + `POST /bom-calculations`; `@ParameterObject` fixes for OpenAPI Pageable documentation (combined JSON box produced unusable sort requests — root cause of the 400/500 debugging session). Tests: `BomDomainTests` and `BomApiTests` (7 API + 3 domain), including same-material quantity update regression. Verified: suite passes 41/41; manual and diagnostic curl confirmed replace, in-place update, and 100-shirts calculation (`"150.000000"`).

## INV-001: Stock Balances and Movement Ledger

- **Type:** Domain feature (Inventory)
- **Evidence:** `V7__inventory_core.sql` (seeded MAIN location; `inventory_item` with material/product shape CHECK and partial unique indexes; `stock_balance` with non-negative checks and `@Version`; two-level append-only ledger: `inventory_transaction` (who/when/why/reference) + `inventory_movement` (per-item deltas)). `StockBalance` domain invariants: receive/adjust/reserve/release/consume with negative-stock and over-reservation rejection; movements have no mutation methods (append-only in the type system). `InventoryService` crosses to Catalog only through `MaterialService` (`requireActiveMaterial`, `MaterialSummary` — no entity/repository leakage); receipts and adjustments commit balance + transaction + movements in one `@Transactional`; actor attribution via `@AuthenticationPrincipal`. REST API: `POST /receipts`, `POST /adjustments`, `GET /balances`, `GET /balances/{id}`, `GET /movements` (ordered history via join query). Tests: `InventoryDomainTests` (5) + `InventoryApiTests` (7) covering ledger accumulation, adjustment rejection below zero, archived-material rejection, role matrix (INVENTORY_MANAGER yes / PLANNER 403), and empty history. Verified: suite passes 53/53; developer manually produced a receipt and adjustment and reviewed the resulting ledger in Swagger.
